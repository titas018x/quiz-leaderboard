...
Clone and run:
bashgit clone https://github.com/your-username/quiz-leaderboard.git
cd quiz-leaderboard
...
javastatic final String REG = "RA2311003010438"; // your reg number
...
Loop poll index 0–9, hit GET /quiz/messages?regNo=...&poll=N
Wait 5 seconds between each call (API requirement)
For each event in the response, check if roundId|participant was already processed
If yes → skip. If no → add score to that participant's total
Sort by score descending
POST the leaderboard to /quiz/submit exactly once
